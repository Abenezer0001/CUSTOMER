...
__export(src_exports, {
  markdownLookBack: () => markdownLookBack
});
...
var markdownToAst = (markdown) => {
  return (0, import_mdast_util_from_markdown.fromMarkdown)(markdown, {
...
var astToMarkdown = (markdownAst) => {
  ...
  return (0, import_mdast_util_to_markdown.toMarkdown)(markdownAst, { extensions: [(0, import_mdast_util_gfm.gfmToMarkdown)()] });
};
...
var preserveWhitespaceInAst = (root) => {
  (0, import_unist_util_visit.visit)(root, "text", (node, index, parent) => {
    if (parent && ["emphasis", "strong", "delete"].includes(parent.type)) {
      const indexWithinParent = parent.children.findIndex((n) => {
        return (0, import_remeda.isDeepEqual)(n.position, node.position);
      });
